...
//       // Note: If your backend route uses router.put('/:id/status'), change .patch to .put
...
//       toast.success(res.data?.message || `Task status updated to "${newStatus}"`);
//       setRepairs((prev) =>
//         prev.map((item) =>
//           item._id === repairId ? { ...item, status: newStatus } : item
//         )
//       );
//     } catch (err) {
//       // Logs the specific backend validation message to browser console
//       console.error("400 Error Details:", err.response?.data);
//       const errorMessage =
//         err.response?.data?.message ||
//         err.response?.data?.error ||
//         "Error updating repair status";
...
//   // Filter ONLY the repairs assigned to THIS logged-in technician
//   const myAssignedJobs = useMemo(() => {
//     return repairs.filter((repair) => {
//       // 1. Match by ObjectId if assignedTechnician is populated or stored as ID
//       const assignedId =
...
//       // 2. Match by technicianName string
//       const recordName = (repair.technicianName || "").trim().toLowerCase();
//       if (loggedInFullName && recordName) {
//         if (
//           recordName === loggedInFullName ||
...
//   const getStatusClass = (status) =>
//     status
//       ? `tat-status-${String(status).toLowerCase().replaceAll(" ", "-")}`
//       : "tat-status-received";
...
//                     "received",
//                     "waiting for parts",
//                   ].includes(String(r.status || "received").toLowerCase())
//                 ).length
//               }
//             </strong>
//           </div>
...
//                           <FiPhone /> {job.customerPhone}
//                         </span>
//                         {job.customerEmail && (
//                           <span className="tat-sub-txt">
//                             <FiMail /> {job.customerEmail}
//                           </span>
//                         )}
//                       </div>
//                     </td>
